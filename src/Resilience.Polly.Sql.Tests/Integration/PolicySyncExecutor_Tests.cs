...
namespace Resilience.Polly.Sql.Tests.Integration
{
    ...
    public class PolicySyncExecutor_Tests
    {
        ...
        private bool DoSomethingSync(int defaultTime = 500)
        {
            Thread.Sleep(TimeSpan.FromMilliseconds(defaultTime));
        ...
        private bool DoSomethingWithTransactionSync(int defaultTime = 500)
        {
            Thread.Sleep(TimeSpan.FromMilliseconds(defaultTime));
        ...
        private bool DoSomethingToBreakTheCircuitSync()
        {
            ++_retriesCounter;
        ...
        private bool DoSomethingToBreakTheCircuitAndContinueSync()
        {
            Thread.Sleep(TimeSpan.FromMilliseconds(500));
        ...
        private bool DoSomethingWithTransactionToBreakTheCircuitSync()
        {
            ++_retriesCounter;
            Thread.Sleep(TimeSpan.FromMilliseconds(500));
        ...
        private bool DoSomethingWithTransactionToBreakTheCircuitAndContinueSync()
        {
            Thread.Sleep(TimeSpan.FromMilliseconds(500));
        ...
        [Test]
        [Category("resilientSyncPolicy.Builder")]
        ...
        [Test]
        [Category("resilientSyncPolicy.WithDefaultPolicies")]
        ...
        [Test]
        [Category("resilientSyncPolicy.WithDefaultPolicies.WithTransaction")]
        ...
        public void Should_Retry_Transaction_Errors_Three_Times_The_Same_Exception_And_The_Fourth_Attempt_Break_The_Circuit_And_Raise_A_BrokenCircuitException()
        {
            ...
            var ex = Assert.Throws<BrokenCircuitException>(() =>
            {
                ...
                {
                    // will break the circuit since throws 3 times in a row the same exception
                    DoSomethingWithTransactionToBreakTheCircuitSync();
                });
            });
        ...
        [Test]
        [Category("resilientSyncPolicy.WithDefaultPolicies.WithTransaction")]
        public void Should_Transaction_Errors_Break_The_Circuit_And_Wait_For_The_Break_And_Try_Again_And_Does_Not_Raise_An_Exception()
        {
            var expectedRetries = 3;
            ...
            var watch = Stopwatch.StartNew();
            var exFirstAttempt = Assert.Throws<BrokenCircuitException>(() =>
            {
                resilientSyncPolicy.Execute(() =>
                {
                    // will break the circuit since throws 3 times in a row the same exception
                    DoSomethingWithTransactionToBreakTheCircuitAndContinueSync();
                });
            });

            var watch2 = Stopwatch.StartNew();
            var exSecondAttempt = Assert.Throws<BrokenCircuitException>(() =>
            {
                resilientSyncPolicy.Execute(() =>
                {
        ...
        [Test]
        [Category("resilientSyncPolicy.WithDefaultPolicies.WithTransaction.WithFallback")]
        public void Should_Fallback_After_Timeout_Exception()
        {
            var result = false;
            var expectedRetries = 0;
            var expectedTimeout = GetExpectedTimeout(5);
            ...
            resilientSyncPolicy.Execute(() =>
            {
                result = DoSomethingWithTransactionSync(120000);
            });
        ...
        [Test]
        [Category("resilientSyncPolicy.WithDefaultPolicies.WithTransaction.WithFallback")]
        public void Should_Fallback_After_Circuit_Breaker_Exception()
        {
            var result = false;
            var expectedRetries = 3;
        ...
        [Test]
        [Category("resilientSyncPolicy.WithDefaultPolicies.WithTimeoutPerRetry")]
        ...
        [Test]
        [Category("resilientSyncPolicy.UseSyncExecutorWithSharedPolicies")]
        public void Should_Break_The_Circuit_Across_Requests_And_Fail_Faster_Then_Wait_For_The_Break_And_Try_Again_Then_Does_Not_Raise_An_Exception()
        {
            Thread.Sleep(TimeSpan.FromSeconds(30));

            var builderFirstRequest = new SqlPolicyBuilder();
            var resilientSyncPolicyFirstRequest = builderFirstRequest
                .UseSyncExecutorWithSharedPolicies()
                .WithDefaultPolicies()
                .Build();

            var exFirstAttempt = Assert.Throws<BrokenCircuitException>(() =>
            {
                resilientSyncPolicyFirstRequest.Execute(() =>
                {
                    // will break the circuit since throws 3 times in a row the same exception
                    DoSomethingToBreakTheCircuitAndContinueSync();
                });
            });

            // simulates a different request
            var builderSecondRequest = new SqlPolicyBuilder();
            var UseSyncExecutorWithSharedPolicies = builderSecondRequest
                .UseSyncExecutorWithSharedPolicies()
                .WithDefaultPolicies()
                .Build();
            ...
            var exSecondAttempt = Assert.Throws<BrokenCircuitException>(() =>
            {
                UseSyncExecutorWithSharedPolicies.Execute(() =>
                {
                    // will fail immediately
                    DoSomethingToBreakTheCircuitAndContinueSync();
                });
            ...
            var resilientSyncPolicyThirdRequest = builderThirdRequest
                .UseSyncExecutorWithSharedPolicies()
                .WithDefaultPolicies()
                .Build();

            // will executes successfully because the circuit is half-open now
            var result = resilientSyncPolicyThirdRequest.Execute(() =>
            {
                DoSomethingToBreakTheCircuitAndContinueSync();
                return true;
            });
